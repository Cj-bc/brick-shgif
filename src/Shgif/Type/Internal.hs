...
-- | Get Shgif's canvas width
--
-- **Note**: use this instead of 'width', if you just want to get width.
--
-- This support both 'Shgif' and 'Container' value constructors.
shgifWidth :: Getter Shgif Int
shgifWidth = to (view _1 . shgifSize)

-- | Get Shgif's canvas height
--
-- **Note**: use this instead of 'height', if you just want to get height.
--
-- This support both 'Shgif' and 'Container' value constructors.
shgifHeight :: Getter Shgif Int
shgifHeight = to (view _2 . shgifSize)


lastTimeStamp :: Getter Shgif Int
lastTimeStamp = to f
    where
        f sgf@(Shgif _ _ _ _ _ _ _ _) = maximum . fmap fst $ sgf^.shgifData
        f c@(Container _ _ sgfs _ _)  = maximum . fmap f . fmap snd $ sgfs
...
mergeToBigCanvas ss = do
    ...
    where
        w = maximum $ fmap (\((w',_), s) -> w' + s^.shgifWidth) ss
        h = maximum $ fmap (\((_,h'), s) -> h' + s^.shgifHeight) ss
